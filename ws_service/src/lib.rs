...
mod tests {
    ...
    /// Test Actor
    struct TestActor<
        T: 'static + Serialize + DeserializeOwned + Send + Sync + std::fmt::Debug + Clone,
    > {
        id: &'static str,
    ...
    impl<T> TestActor<T>
    where
        T: 'static + Serialize + DeserializeOwned + Send + Sync + std::fmt::Debug + Clone + Unpin,
    {
    ...
    where
        T: 'static + Serialize + DeserializeOwned + Send + Sync + std::fmt::Debug + Unpin + Clone,
    {
        type Context = Context<Self>;
    ...
    impl<T> Handler<Signal<T>> for TestActor<T>
    where
        T: 'static + Serialize + DeserializeOwned + Send + Sync + std::fmt::Debug + Unpin + Clone,
    {
        type Result = ();

        fn handle(&mut self, sig: Signal<T>, ctx: &mut Self::Context) -> Self::Result {
            trace!("{} -- received message : {:?}", self.id, sig);
