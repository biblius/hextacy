...
fn quote_repo_access(
    ...
    let Some(driver_field) = fields.driver_fields.get(connection.name) else { return quote!() };

    let _type: TypePath = syn::parse_str(connection.full_path).unwrap();
