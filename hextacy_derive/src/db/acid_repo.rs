...
fn quote_pg_atomic_access(
    ...
    quote!(
        #[async_trait::async_trait(?Send)]
         impl #impl_generics ::hextacy::db::AcidRepositoryAccess<#__type> for #ident #ty_generics #where_clause {
            async fn connect<'a>(&'a self) -> Result<hextacy::db::AtomicConnection<#__type>, hextacy::db::DatabaseError> {
                let tx = self.tx_pg.borrow_mut();
                match *tx {
                    Some(ref _tx) => Ok(::hextacy::db::AtomicConnection::Existing(tx)),
                    None => {
                        let conn = self.postgres.connect().await?;
                        Ok(hextacy::db::AtomicConnection::New(conn))
                    },
                }
            }
         }
...
fn quote_atomic_access(
    ...
    quote!(
        #[async_trait::async_trait(?Send)]
        impl #impl_generics ::hextacy::db::Atomic for #ident #ty_generics #where_clause {
            async fn start_transaction(&self) -> Result<(), hextacy::db::DatabaseError> {
                #start
                Ok(())
            }
            async fn commit_transaction(&self) -> Result<(), hextacy::db::DatabaseError> {
                #commit
                Ok(())
            }
            async fn rollback_transaction(&self) -> Result<(), hextacy::db::DatabaseError> {
                #abort
                Ok(())
            }
        }
...
fn quote_atomic_start(has_pg: bool, has_mg: bool) -> proc_macro2::TokenStream {
    let pg = quote!(
            ...
            match *tx {
                Some(_) => return Err(hextacy::db::DatabaseError::Transaction(::hextacy::db::TransactionError::InProgress)),
                None => {
    ...
    let mg = quote!(
        ...
        match *tx {
            Some(_) => {
                return Err(hextacy::db::DatabaseError::Transaction(
                    ::hextacy::db::TransactionError::InProgress,
                ))
            },
            None => {
        ...
        match tx.take() {
            Some(ref mut conn) => {
                diesel::connection::AnsiTransactionManager::commit_transaction(&mut **conn)
                    .map_err(hextacy::db::DatabaseError::from)?;
            },
            None => return Err(hextacy::db::DatabaseError::Transaction(
                ::hextacy::db::TransactionError::NonExisting,
            )
            .into()),
        };
    );
