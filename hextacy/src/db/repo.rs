...
macro_rules! repository {
    (
      ...
      $($id:ident => $repo_bound:path),*
         ...
         {
            $(
                pub $field: hextacy::clients::db::Client<$client, $conn_name>,
            )+
             ...
             $($id: $repo_bound),*
         {
             pub fn new($($client: ::std::sync::Arc<$client>),+) -> Self {
                 Self {
                    $(
                        $field: hextacy::clients::db::Client::new($client),
                    )+
...
macro_rules! acid_repo {
    (
      ...
      $($id:ident => $repo_bound:path),*
           ...
           {
              $(
                  $field: hextacy::clients::db::Client<$client, $conn_name>,
                  $tx: hextacy::db::Transaction<$conn_name>,
              )+
               $($id: ::std::marker::PhantomData<$id>),*
           }
               ...
               $($id: $repo_bound),*
           {
               pub fn new($($client: ::std::sync::Arc<$client>),+) -> Self {
                   Self {
                      $(
                          $field: hextacy::clients::db::Client::new($client),
                          $tx: hextacy::db::Transaction::new(None),
                      )+
                       $($id: ::std::marker::PhantomData),*
                   }
               }
