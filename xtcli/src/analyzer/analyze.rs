...
pub enum XtcFileType {
    Setup,
    Handler,
    Data,
}

/// Analyze the router directory and generate an xtc.yaml/json file
#[derive(Debug, Args)]
...
pub fn router_read_recursive(
    dir: &Path,
    scan: &mut ScanResult,
    callback: &dyn Fn(&DirEntry, XtcFileType) -> Result<FileScanResult, AlxError>,
    dir_type: Option<XtcFileType>,
) -> Result<(), AlxError> {
    print(&format!(
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            if path.ends_with("handler") {
                router_read_recursive(&path, scan, callback, Some(XtcFileType::Handler))?;
            } else if path.ends_with("setup") {
                router_read_recursive(&path, scan, callback, Some(XtcFileType::Setup))?;
            } else if path.ends_with("data") {
                router_read_recursive(&path, scan, callback, Some(XtcFileType::Data))?;
            } else {
            ...
            let in_setup = dir_type.is_some() && dir_type.unwrap() == XtcFileType::Setup;

            if file_name.contains("setup") || in_setup {
                let routes = callback(&entry, XtcFileType::Setup)?;
                if let FileScanResult::Routes(routes) = routes {
                    scan.routes.insert(ep_name.to_string(), routes);
                }
            }
            if file_name.contains("handler") {
                let handlers = callback(&entry, XtcFileType::Handler)?;
            ...
            if file_name.contains("data") {
                let data = callback(&entry, XtcFileType::Data)?;
...
pub fn analyze(entry: &DirEntry, file_type: XtcFileType) -> Result<FileScanResult, AlxError> {
