...
pub fn handle_gen_route(args: GenerateArgs, router_path: &str) {
    ...
    for file in ROUTE_FILES {
        ...
        match file {
            "api" => boiler::apis(&mut contents, &apis, BoilerType::Route),
            "domain" => boiler::domain(&mut contents, &service_name, &apis),
            "infrastructure" if !apis.is_empty() => boiler::infrastructure(&mut contents, &apis),
            "setup" => boiler::router::setup(&mut contents, &service_name, &apis),
...
pub fn handle_gen_mw(args: GenerateArgs, mw_path: &str) {
    ...
    for file in MW_FILES {
        ...
        let mut contents = String::new();
        match file {
            "api" => boiler::apis(&mut contents, &apis, BoilerType::MW),
            "domain" => boiler::domain(&mut contents, &service_name, &apis),
            "infrastructure" if !apis.is_empty() => boiler::infrastructure(&mut contents, &apis),
            "interceptor" => {
                boiler::middleware::mw_interceptor(&mut contents, &service_name, &apis)
            }
