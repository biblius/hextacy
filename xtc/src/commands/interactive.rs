...
impl AlxApp {
    fn new() -> Self {
        use CommandProxy::*;
        let commands = vec![Generate, Migration, Crypto, Analyze, Envex];
        let mut this = Self {
        ...
        match active {
            OptionsProxy::GenerateR(ref mut opts) | OptionsProxy::GenerateMW(ref mut opts) => {
                match opt {
                    ...
                    OptionField::Contracts => push_or_insert(&mut opts.contracts, ch),
        ...
        match active {
            OptionsProxy::GenerateR(ref mut opts) | OptionsProxy::GenerateMW(ref mut opts) => {
                match opt {
                    ...
                    OptionField::Contracts => pop_opt(&mut opts.contracts),
    ...
    fn update_option_list(&mut self, command: SubcommandProxy) {
        ...
        use SubcommandProxy::*;
        match command {
            Generate(_) => self.options.items = vec![Name, Contracts, Path],
...
fn create_opt_list_item<'a>(app: &AlxApp, opt: &OptionField, width: u16) -> ListItem<'a> {
    ...
    let value = match active {
        OptionsProxy::GenerateR(opts) | OptionsProxy::GenerateMW(opts) => match opt {
            ...
            OptionField::Contracts => opts.contracts.clone().unwrap_or_default(),
...
fn subcommand_list<B: Backend>(frame: &mut Frame<B>, app: &mut AlxApp, command: CommandProxy) {
    ...
    let subcommands: Vec<ListItem> = app
        ...
        .map(|cmd| {
            ...
            let block_width = middle.width;
            let selected_style = if app.active_sub.is_some() && *cmd == app.active_sub.unwrap() {
                Spans::from(vec![Span::styled(
                    format!(
                        "{}{}>",
                        cmd.description(),
                        "=".repeat(block_width as usize - 1 - cmd.description().len()),
                    ),
