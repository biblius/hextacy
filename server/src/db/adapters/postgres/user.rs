use crate::db::adapters::postgres::schema::users;
...
use hextacy::clients::{db::postgres::PgPoolConnection, oauth::OAuthProvider};
...
impl UserRepository<PgPoolConnection> for PgUserAdapter {
    ...
    async fn get_by_oauth_id(
        ...
        match provider {
            hextacy::clients::oauth::OAuthProvider::Google => {
                query = query.filter(google_id.eq(oauth_id))
            }
            hextacy::clients::oauth::OAuthProvider::Github => {
